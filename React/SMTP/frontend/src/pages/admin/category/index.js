...
export default function Category() {
  ...
  const newCatSave = async () => {
    ...
    Axios.post("/category/create", formData, {}).then((res) => {
  ...
  const editCat = (id) => {
    setEditCatId(id);
    Axios.get("/category/edit/" + id).then(async (response) => {
  ...
  const finalDelCat = () => {
    handleDelCatClose();
    Axios.delete("/category/delete/" + delCatId).then(async (response) => {
  ...
  const catUpdate = async () => {
    ...
    Axios.put("/category/update", formData, {}).then(async (res) => {
      if (res.data.status === 200) {
        handleEditCatClose();
