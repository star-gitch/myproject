...
export default function SubCategory() {
  ...
  const submit = () => {
    ...
    Axios.post("http://localhost:5000/subcat/create", formData, {}).then(
      (res) => {
        if (res.data.status === 200) {
          ...
          setSubPdf("");
          // Get all subcategory
          Axios.get("http://localhost:5000/subcat/all").then(
            async (response) => {
              setSubCatList(response.data);
            }
          );
  ...
  const showEditModal = (id) => {
    setEditSubId(id);
    Axios.get("http://localhost:5000/subcat/edit/" + id).then(
      async (response) => {
  ...
  const finalDelSub = () => {
    setOpenDelSub(false);
    Axios.delete("http://localhost:5000/subcat/delete/" + delSubId).then(
      async (response) => {
  ...
  const editSubmit = () => {
    ...
    Axios.put("http://localhost:5000/subcat/update", formData, {}).then(
      (res) => {
        if (res.data.status === 200) {
